...
console.log(__dirname+"/images/Photo.jpg");
*/
app.get('/ITtechnologies', function(request, response) {
    //response.send("<h2><center>Welcome to Node JS app</h2>");
    response.write("<h2><center><u>Node JS  Application </u></center></h2>");
	
    response.write("<h2><center>Welcome to  Al-Hassan Technologies. Please Contact +91-9866432640 for more information or send an email to technologiestraininghyd@gmail.com <center></h2>" );
    response.end();
...
app.get("/html", function(req,res){
    ...
    app.get("/jsonData", function(req,res){
        ...
        //res.json({'name': 'Shaik Jahangir'});
        res.send({
		'name': 'IT Technologies',
		'technology': 'DevOps',
		'contact' : '9866432640',
		'email': 'technologiestraininghyd@gmail.com'
	            });
...
app.get('/redirect', function(req, res) {
    //Send status 300
        res.redirect('http://ittechnologies.com');
    });
    
 
    app.listen(app.get('port'), function() {
        console.log("Node JS app is running at http://localhost:" + app.get('port') +"/ITtechnologies");
      })
